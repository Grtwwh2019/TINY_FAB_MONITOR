...
- 提供按需“耗时分析”：整体 delay 只比较指定结束作业在各自业务日期中的 R 完成时刻，并支持前一个完成日期、指定日期或最近日期平均基准。
- 指定启动作业在当天和全部基准日期都必须具有真实 R；启动 R 继续用于批次耗时、启动对齐应完成时间、ETA 和慢点分析，但不改变整体 delay 结论。
- 跨午夜完成按业务时刻连续计算（例如次日 02:00 记为第 26 小时）；多日基准平均的是结束作业业务完成时刻，不直接相减不同日期的完整时间戳。
