...
- 提供按需“耗时分析”：可选择指定单个基准日期、前一个完整日期或最近多个完整日期平均；整体 delay 只比较指定结束作业的有效 R 完成时刻。
- 指定启动作业的 Level 必须大于等于40，并在分析日及基准日具有真实 R；启动 R 用于批次内部对齐和“应完成时间”，不会改变整体 delay 结论。
- 任务完成偏移精确拆成“依赖就绪偏移 + 就绪后完成间隔差”；只使用 R 时间，不读取或推算 I，也不自动认定执行、等待或同 Thread 阻塞原因。
- 最晚完成的直接依赖作为当天实际就绪驱动，并可沿真实依赖向上递归追踪；中间任务可以是任意 Level ≥ 40，无需逐级或直接连接 Level 40。W、I 和占位 R 都会继续递归，有效真实 R 作为精确锚点停止；Level 40 是固定边界，并列分支全部保留。
