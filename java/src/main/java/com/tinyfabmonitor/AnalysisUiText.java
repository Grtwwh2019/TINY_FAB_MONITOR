...
final class AnalysisUiText {
    ...
    static String help() {
        return "耗时分析指标说明（纯 R 模式）\n\n" +
            ...
            "依赖就绪时间\n所有直接前置依赖中最晚的有效 R 时间。并列最晚的依赖全部保留。依赖缺失、未 R、占位时间或同一 FAB 映射到多个任务时，不强行计算。Level 20 路径在递归追踪中截止。\n\n" +
            "精确分解\n当依赖数据完整时：启动对齐完成差 = 依赖就绪偏移 + 就绪后完成间隔差。这是 R 时间的数学分解，不代表已经判断出任务实际执行慢、调度等待或同 Thread 阻塞。\n\n" +
            "关键前置依赖与递归链\n从当天实际决定依赖就绪的前置任务向上递归，并列分支全部保留；遇到启动作业、Level 20、无更多依赖、循环、数据不足或映射歧义时停止。右键任务可查看完整链和缺失项。\n\n" +
            "关注阈值\n只影响高亮和人工核对建议，不改变任何原始差值，也不改变结束作业的整体 delay 结论。\n\n" +
            "数据边界\n本页不读取或推算 I 时间，不估算未完成任务 ETA，不自动判断等待原因或同 Thread 阻塞。发现 R 时间偏移后，应按建议去人工核对日志或其他数据。";
    }
