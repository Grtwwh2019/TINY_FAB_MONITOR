...
final class AnalysisUiText {
    ...
    static String help() {
        return "耗时分析指标说明（纯 R 模式）\n\n" +
            "基准方式\n可选择指定单个业务日期、自动选择前一个完整业务日期，或使用最近2–30个完整业务日期的平均值。多日模式只采用启动作业和结束作业都有有效真实 R 的日期，并在结果中列出实际日期。\n\n" +
            "整体 delay\n只比较指定结束作业在分析日期和基准日期中的有效 R 完成时刻。分析日期更晚就是 delay；跨午夜按连续时间处理。\n\n" +
            "启动对齐应完成时间\n当前启动作业 R + 基准批次耗时。启动作业 Level 必须大于等于40。它用于批次内部对齐和进度参照，不产生第二个 delay 结论；启动作业缺少真实 R 时不分析。\n\n" +
