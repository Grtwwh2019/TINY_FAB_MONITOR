...
final class AppConfig {
    ...
    private AppConfig(Path baseDirectory, Properties p) {
        ...
        scheduleTable = table(p, "tables.schedule");
        levelDescTable = table(p, "tables.level_desc");
        fabPlanTable = table(p, "tables.fab_plan");
    ...
    private static String table(Properties p, String key) {
        String value = trim(p.getProperty(key));
        if (value.isEmpty()) throw new IllegalArgumentException(key + " 不能为空，请填写实际业务表名");
